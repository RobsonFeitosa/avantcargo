    ...
    <nav className={`fixed border-b-4 top-0 left-0 w-full z-50 py-2 transition-all bg-white duration-300 ${scrolled ? "bg-white " : "bg-white"}`}>
      <div className="container flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          <img src={logo} alt="AvantCargo Logo" className="h-14 w-auto object-contain" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-10">
          {navItems.map((item) => (
            <Link
              key={item.name}
              to={item.path}
              className={`text-sm font-bold tracking-tight transition-all duration-300 relative group flex items-center px-4 py-2 rounded-sm ${location.pathname === item.path ? "text-primary bg-primary/10" : "text-foreground/90 hover:text-primary hover:bg-primary/5"}`}
            >
              {item.name}
            </Link>
          ))}
