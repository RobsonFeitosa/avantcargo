...
export default function CargoTrackingConfigPage() {
  const { user } = useAuth();
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => cargoTrackingActions.update(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cargo-tracking-config"] });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight text-emerald-950">Rastreamento de Carga</h1>
        <p className="text-emerald-900/60 font-medium">
          Configure o título e o texto de descrição da página de Rastreamento de Carga.
        </p>
      </div>
        ...
        <Card className="border-none shadow-sm overflow-hidden">
          <CardHeader className="bg-emerald-50/50 border-b border-emerald-100">
            ...
            <CardDescription className="text-emerald-800/60 font-medium">
              Título e descrição exibidos na página pública de rastreamento.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-emerald-900/70 font-semibold uppercase text-[10px] tracking-wider">Descrição / Instruções</Label>
              <Textarea
                maxLength={1000}
                value={description}
            ...
            <div className="flex justify-end gap-4 pt-4 mt-2 border-t border-emerald-50">
              <Button
                ...
                onClick={() => queryClient.invalidateQueries({ queryKey: ["cargo-tracking-config"] })}
              >
                Descartar
              </Button>
              <Button
